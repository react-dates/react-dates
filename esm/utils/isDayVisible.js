...
export default function isDayVisible(day, month, numberOfMonths, enableOutsideDays) {
  ...
  var startKey = toISOMonthString(month);
  ...
  if (enableOutsideDays) {
    ...
    if (!endCacheOutsideDays.has(endKey)) {
      endCacheOutsideDays.set(endKey, month.clone().endOf('week').add(numberOfMonths - 1, 'months').endOf('month').endOf('week').hour(12));
    }
  ...
  if (!endCacheInsideDays.has(endKey)) {
    endCacheInsideDays.set(endKey, month.clone().add(numberOfMonths - 1, 'months').endOf('month').hour(12));
  }
