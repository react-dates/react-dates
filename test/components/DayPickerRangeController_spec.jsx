...
function getCallsByModifier(stub, modifier) {
  return stub
    .getCalls()
    .filter((call) => call.args[call.args.length - 1] === modifier);
}
...
describe("DayPickerRangeController", () => {
    ...
    describe("phrases", () => {
      ...
      describe("focusedInput is START_DATE", () => {
        it("state.phrases.chooseAvailableDate equals props.phrases.chooseAvailableStartDate", () => {
          ...
          const newAvailableDatePhrase =
            wrapper.state().phrases.chooseAvailableDate;
          expect(newAvailableDatePhrase).to.equal(
    ...
    describe("rebuilding currentMonth/visibleDays", () => {
      describe("initialVisibleMonth changed", () => {
        describe("focusedInput has changed and is truthy", () => {
          it("calls getStateForNewMonth with nextProps", () => {
            ...
            const wrapper = shallow(
              <DayPickerRangeController {...props} focusedInput={null} />
            );
          ...
          it("sets state.currentMonth to getStateForNewMonth.currentMonth", () => {
            ...
            expect(wrapper.instance().state.currentMonth).to.equal(
              currentMonth
            );
          });
      ...
      describe("startDate changed from one date to another", () => {
        it("removes previous `after-hovered-start` range", () => {
          ...
          const firstAvailableDate = startDate
            .clone()
            .add(minimumNights + 1, "days");
          const deleteModifierFromRangeSpy = sinon.spy(
            DayPickerRangeController.prototype,
          ...
          const afterHoverStartCalls = getCallsByModifier(
            deleteModifierFromRangeSpy,
            "after-hovered-start"
          );
          expect(afterHoverStartCalls.length).to.equal(1);
          expect(
            isSameDay(afterHoverStartCalls[0].args[1], dayAfterStartDate)
          ).to.equal(true);
          expect(
            isSameDay(afterHoverStartCalls[0].args[2], firstAvailableDate)
          ).to.equal(true);
        });
      });
      ...
      describe("endDate changed from one date to another", () => {
        it("removes previous `selected-end-no-selected-start` when no start date selected", () => {
          ...
          const selectedEndNoStartDateDelete = getCallsByModifier(
            deleteModifierSpy,
            "selected-end-no-selected-start"
          );
          expect(selectedEndNoStartDateDelete.length).to.equal(1);
          expect(
            isSameDay(selectedEndNoStartDateDelete[0].args[1], endDate)
          ).to.equal(true);

          const selectedEndNoStartDateAdd = getCallsByModifier(
            addModifierSpy,
            "selected-end-no-selected-start"
          );
          expect(selectedEndNoStartDateAdd.length).to.equal(1);
          expect(
            isSameDay(selectedEndNoStartDateAdd[0].args[1], nextEndDate)
          ).to.equal(true);
        });
      ...
      describe("selected-start modifier", () => {
        describe("props.startDate did not change", () => {
          it("does not call this.addModifier with `selected-start", () => {
            ...
            wrapper
              .instance()
              .UNSAFE_componentWillReceiveProps({ ...props, startDate });
            expect(
              getCallsByModifier(addModifierSpy, "selected-start").length
            ).to.equal(0);
          });
          ...
          it("does not call this.deleteModifier with `selected-start", () => {
            ...
            wrapper
              ...
              .UNSAFE_componentWillReceiveProps({ ...props, startDate });
            expect(
              getCallsByModifier(deleteModifierSpy, "selected-start").length
            ).to.equal(0);
          });
        });
        ...
        describe("props.startDate changed", () => {
          it("deleteModifier gets called with old startDate and `selected-start`", () => {
            ...
            const selectedStartCalls = getCallsByModifier(
              deleteModifierSpy,
              "selected-start"
            );
      ...
      describe("selected-end modifier", () => {
        describe("props.endDate did not change", () => {
          it("does not call this.addModifier with `selected-end`", () => {
            ...
            wrapper
              .instance()
              .UNSAFE_componentWillReceiveProps({ ...props, endDate });
            expect(
              getCallsByModifier(addModifierSpy, "selected-end").length
            ).to.equal(0);
          });
          ...
          it("does not call this.deleteModifier with `selected-end`", () => {
            ...
            wrapper
              ...
              .UNSAFE_componentWillReceiveProps({ ...props, endDate });
            expect(
              getCallsByModifier(deleteModifierSpy, "selected-end").length
            ).to.equal(0);
          });
        });
        ...
        describe("props.endDate changed", () => {
          it("deleteModifier gets called with old endDate and `selected-end`", () => {
            ...
            const selectedEndCalls = getCallsByModifier(
              deleteModifierSpy,
              "selected-end"
            );
        ...
        describe("startDate changed", () => {
          describe("new startDate does not exist", () => {
            it("deleteModifierFromRange does not get called with `hovered-span`", () => {
              ...
              const wrapper = shallow(
                <DayPickerRangeController {...props} startDate={today} />
              );
              wrapper
                .instance()
                .UNSAFE_componentWillReceiveProps({
              ...
              const hoverSpanCalls = getCallsByModifier(
                deleteModifierFromRangeSpy,
                "hovered-span"
              );
          ...
          describe("new startDate and new endDate both exist", () => {
            it("deleteModifierFromRange gets called with startDate, endDate + 1 day, and `hovered-span`", () => {
              ...
              expect(
                isSameDay(hoverSpanCalls[0].args[2], dayAfterEndDate)
              ).to.equal(true);
            });
          });
        });
        ...
        describe("startDate changed", () => {
          describe("old startDate and old endDate both exist", () => {
            it("deleteModifierFromRange gets called with old startDate + 1 day, old endDate, and `selected-span`", () => {
              ...
              const selectedSpanCalls = getCallsByModifier(
                deleteModifierFromRangeSpy,
                "selected-span"
              );
              ...
              expect(
                isSameDay(selectedSpanCalls[0].args[2], dayAfterEndDate)
              ).to.equal(true);
            });
          });
          ...
          describe("new startDate and new endDate both exist", () => {
            it("addModifierToRange gets calls with new startDate + 1 day, endDate, and `selected-span`", () => {
              const addModifierToRangeSpy = sinon.spy(
                DayPickerRangeController.prototype,
                "addModifierToRange"
              );
              ...
              const selectedStartCalls = getCallsByModifier(
                addModifierToRangeSpy,
                "selected-span"
              );
              expect(selectedStartCalls.length).to.equal(1);
              expect(
                isSameDay(selectedStartCalls[0].args[1], dayAfterStartDate)
              ).to.equal(true);
        ...
        describe("endDate changed", () => {
          ...
          describe("new startDate and new endDate both exist", () => {
            it("addModifierToRange gets calls with startDate + 1 day, endDate, and `selected-span`", () => {
              ...
              expect(selectedSpanCalls.length).to.equal(1);
              expect(
                isSameDay(selectedSpanCalls[0].args[1], dayAfterStartDate)
              ).to.equal(true);
      ...
      describe("after-hovered-start modifier", () => {
        describe("start date changed, is truthy, and there is no end date", () => {
          ...
          it("`after-hovered-start` addModifierToRange has span beginning with day after startDate", () => {
            ...
            expect(toISODateString(afterHoverStartCalls[0].args[1])).to.equal(
              startSpan
            );
          });
          ...
          it("`after-hovered-start` addModifierToRange has span ending with startDate + minimumNights + 1", () => {
            ...
            const startDate = moment();
            const endSpan = toISODateString(
              startDate.clone().add(minimumNights + 1, "day")
            );
            const wrapper = shallow(
              <DayPickerRangeController
                {...props}
            ...
            expect(toISODateString(afterHoverStartCalls[0].args[2])).to.equal(
              endSpan
            );
          });
        });
        ...
        describe("new start date is falsy", () => {
          it("does not call addModifierToRange with `after-hovered-start`", () => {
            ...
            wrapper
              .instance()
              .UNSAFE_componentWillReceiveProps({ ...props, startDate: null });
            const afterHoverStartCalls = getCallsByModifier(
              addModifierToRangeSpy,
              "after-hovered-start"
            );
        ...
        describe("old startDate exists", () => {
          describe("neither startdate nor focusedInput changed", () => {
            it("does not call deleteModifierFromRange with `blocked-minimum-nights`", () => {
              ...
              const minimumNightsCalls = getCallsByModifier(
                deleteModifierFromRangeSpy,
          ...
          describe("startDate changed", () => {
            it("calls deleteModifierFromRange with old start date, + min nights, and `blocked-minimum-nights", () => {
              ...
              const minimumNightsEndSpan = startDate
                .clone()
              ...
              expect(
                isSameDay(minimumNightsCalls[0].args[2], minimumNightsEndSpan)
              ).to.equal(true);
            });
          });
          ...
          describe("new focusedInput !== END_DATE", () => {
            ...
            it("updates state to remove `blocked-minimum-nights` and `blocked` from the appropriate days", () => {
              ...
              for (let i = 0; i < minimumNights; i += 1) {
                ...
                expect(visibleDays[monthString][dateString]).to.include(
                  "blocked-minimum-nights"
                );
                expect(visibleDays[monthString][dateString]).to.include(
                  "blocked"
              ...
              for (let i = 0; i < minimumNights; i += 1) {
                const monthString = toISOMonthString(day);
                const dateString = toISODateString(day);
                expect(newVisibleDays[monthString][dateString]).not.to.include(
                  "blocked-minimum-nights"
                );
                expect(newVisibleDays[monthString][dateString]).not.to.include(
                  "blocked"
      ...
      describe("blocked-out-of-range", () => {
        ...
        describe("focusedInput changed", () => {
          ...
          it("if isOutsideRange(day) is true calls addModifier with `blocked-out-of-range` for each day", () => {
            ...
            const blockedCalendarCalls = getCallsByModifier(
              addModifierSpy,
      ...
      describe("highlighted-calendar", () => {
        ...
        describe("focusedInput changed", () => {
          ...
          it("if isDayHighlighted(day) is true calls addModifier with day and `highlighted-calendar`", () => {
            ...
            const highlightedCalendarCalls = getCallsByModifier(
              addModifierSpy,
        ...
        describe("focusedInput did change", () => {
          ...
          describe("focusedInput === START_DATE", () => {
            it("calls addModifierToRange with `hovered-start-blocked-minimum-nights` if getMinNightsForHoverDate returns a positive integer", () => {
              ...
              const hoveredStartBlockedMinNightsCalls = getCallsByModifier(
                addModifierToRangeSpy,
              ...
              expect(
                isSameDay(
                  hoveredStartBlockedMinNightsCalls[0].args[1],
                  today.clone().add(1, "days")
                )
              ).to.equal(true);
              expect(
                isSameDay(
                  hoveredStartBlockedMinNightsCalls[0].args[2],
        ...
        describe("focusedInput did change", () => {
          ...
          describe("focusedInput === START_DATE", () => {
            it("calls addModifier with `hovered-start-first-possible-end` if getMinNightsForHoverDate returns a positive integer", () => {
              ...
              const hoveredStartFirstPossibleEndCalls = getCallsByModifier(
                addModifierSpy,
              ...
              expect(
                isSameDay(
                  hoveredStartFirstPossibleEndCalls[0].args[1],
                  today.clone().add(2, "days")
                )
              ).to.equal(true);
            });
      ...
      describe("no-selected-start-before-selected-end", () => {
        describe("start or end date has changed, start date is falsey, and end date is truthy", () => {
          it("calls addModifier with `no-selected-start-before-selected-end` if day is before selected end date", () => {
            ...
            const noSelectedStartBeforeSelectedEndCalls = getCallsByModifier(
              addModifierSpy,
        ...
        describe("start date has changed, previous start date is falsey, start and end date is truthy", () => {
          it("calls deleteModifier with `no-selected-start-before-selected-end` if day is before selected end date", () => {
            ...
            const numberOfVisibleDays = Object.values(visibleDays).reduce(
              (total, visibleDayArray) =>
                total + Object.keys(visibleDayArray).length,
              0
            );
            wrapper.instance().UNSAFE_componentWillReceiveProps({
              ...props,
            ...
            expect(noSelectedStartBeforeSelectedEndCalls.length).to.equal(
              numberOfVisibleDays
            );
          });
        });
        ...
        describe("start date is truthy, and end date is falsey", () => {
          it("calls addModifier with `selected-start-no-selected-end`", () => {
            ...
            wrapper
              .instance()
              .UNSAFE_componentWillReceiveProps({ ...props, startDate });
            const selectedStartNoSelectedEndCalls = getCallsByModifier(
              addModifierSpy,
              "selected-start-no-selected-end"
            );
            expect(selectedStartNoSelectedEndCalls.length).to.equal(1);
            expect(selectedStartNoSelectedEndCalls[0].args[1]).to.equal(
              startDate
            );
          });
        });
      ...
      describe("before-hovered-end", () => {
        describe("end date changed, end date is truthy and start date is falsey", () => {
          it("calls addModifierToRange with `before-hovered-end`", () => {
            ...
            const beforeHoveredEndCalls = getCallsByModifier(
              addModifierToRangeSpy,
              "before-hovered-end"
            );
            ...
            expect(toISODateString(beforeHoveredEndCalls[0].args[1])).to.equal(
              toISODateString(
                newEndDate.clone().subtract(minimumNights, "days")
              )
            );
        ...
        describe("start date has changed", () => {
          describe("start and end date are truthy, and previous start date is falsey", () => {
            it("calls deleteModifier with `selected-end-in-hovered-span`", () => {
              ...
              const deleteModifierCalls = getCallsByModifier(
                deleteModifierSpy,
                "selected-end-in-hovered-span"
              );
    ...
    describe("phrases", () => {
      ...
      describe("neither props.focusedInput nor props.phrases have changed", () => {
        it("state.phrases does not change", () => {
          ...
          const wrapper = shallow(
            <DayPickerRangeController {...props} phrases={phrases} />
          );
          wrapper.setState({ phrases: phrasesObject });
          wrapper
            .instance()
            .UNSAFE_componentWillReceiveProps({ ...props, phrases });
      ...
      describe("props.phrases has changed", () => {
        ...
        describe("focusedInput is null", () => {
          it("state.phrases.chooseAvailableDate equals props.phrases.chooseAvailableDate", () => {
            const wrapper = shallow(
              <DayPickerRangeController {...props} phrases={{}} />
            );
            wrapper.setState({ phrases: {} });
            wrapper
              .instance()
  ...
  describe("#onDayClick", () => {
    ...
    describe("props.startDateOffset / props.endDateOffset", () => {
      it("calls props.onDatesChange with startDate === startDateOffset(date) and endDate === endDateOffset(date)", () => {
        ...
        const args = onDatesChangeStub.getCall(0).args[0];
        expect(args.startDate.format()).to.equal(
          clickDate.clone().subtract(2, "days").format()
        );
        expect(args.endDate.format()).to.equal(
          clickDate.clone().add(4, "days").format()
        );
      });
      ...
      it("does not call props.onDatesChange when dateOffset isOutsideRange", () => {
        ...
        const wrapper = shallow(
          <DayPickerRangeController
            ...
            isOutsideRange={(day) =>
              day.isAfter(moment(today).clone().add(1, "days"))
            }
          />
        );
        wrapper.instance().onDayClick(clickDate);
      ...
      it("calls props.onDatesChange with startDate === startDateOffset(date) and endDate === selectedDate when endDateOffset not provided", () => {
        ...
        const args = onDatesChangeStub.getCall(0).args[0];
        expect(args.startDate.format()).to.equal(
          clickDate.clone().subtract(5, "days").format()
        );
      ...
      it("calls props.onDatesChange with startDate === selectedDate and endDate === endDateOffset(date) when startDateOffset not provided", () => {
        ...
        expect(args.endDate.format()).to.equal(
          clickDate.clone().add(12, "days").format()
        );
      });
    });
  ...
  describe("#onDayMouseEnter", () => {
    it("sets state.hoverDate to the day arg", () => {
      const wrapper = shallow(
        <DayPickerRangeController focusedInput={START_DATE} />
      );
    ...
    it("sets state.dateOffset to the start and end date range when range included", () => {
      ...
      wrapper.instance().onDayMouseEnter(today);
      expect(wrapper.state().dateOffset.start.format()).to.equal(
        today.format()
      );
      expect(wrapper.state().dateOffset.end.format()).to.equal(
        today.clone().add(3, "days").format()
      );
    });
      ...
      describe("startDate and !endDate and focusedInput === `END_DATE`", () => {
        describe("old hoverDate is after startDate", () => {
          it("calls deleteModifierFromRange with startDate, old hoverDate and `hovered-span`", () => {
            ...
            expect(hoverSpanCalls[0].args[1]).to.equal(startDate);
            expect(
              isSameDay(hoverSpanCalls[0].args[2], dayAfterHoverDate)
            ).to.equal(true);
          });
        });
      ...
      describe("!startDate and endDate and focusedInput === `START_DATE`", () => {
        describe("old hoverDate is before endDate", () => {
          it("calls deleteModifierFromRange", () => {
            ...
            expect(deleteModifierFromRangeSpy.getCall(0).args[1]).to.equal(
              hoverDate
            );
            expect(deleteModifierFromRangeSpy.getCall(0).args[2]).to.equal(
              endDate
            );
            expect(deleteModifierFromRangeSpy.getCall(0).args[3]).to.equal(
              "hovered-span"
            );
            expect(
              isSameDay(
                ...
                endDate.subtract(
                  DayPickerRangeController.defaultProps.minimumNights,
                  "days"
                )
              )
            ).to.equal(true);
            expect(deleteModifierFromRangeSpy.getCall(1).args[2]).to.equal(
              endDate
            );
            expect(deleteModifierFromRangeSpy.getCall(1).args[3]).to.equal(
              "before-hovered-end"
            );
          });
        });
        ...
        describe("new hoverDate is not blocked and is before endDate", () => {
          it("calls addModifierFromRange", () => {
            ...
            expect(addModifierToRangeSpy.getCall(0).args[1]).to.equal(
              hoverDate
            );
            expect(addModifierToRangeSpy.getCall(0).args[2]).to.equal(endDate);
            expect(addModifierToRangeSpy.getCall(0).args[3]).to.equal(
              "hovered-span"
            );
          });
        });
        ...
        describe("end date is falsey and focusedInput === `END_DATE`", () => {
          ...
          describe("day is not blocked, and is after the start date", () => {
            it("calls addModifier with `selected-start-in-hovered-span` on start date", () => {
              ...
              const addModifierCalls = getCallsByModifier(
                addModifierSpy,
                "selected-start-in-hovered-span"
              );
      ...
      describe("before-hovered-end modifier", () => {
        describe("end date is truthy and focusedInput is truthy", () => {
          it("calls deleteModifierFromRange with `before-hovered-end` on minimum nights days before end date", () => {
            ...
            const minimumNightStartSpan = endDate
              .clone()
              .subtract(minimumNights, "days");
            deleteModifierFromRangeSpy.resetHistory();
            wrapper.instance().onDayMouseEnter(today);
            ...
            expect(
              toISODateString(deleteModifierFromRangeCalls[0].args[1])
            ).to.equal(toISODateString(minimumNightStartSpan));
        ...
        describe("day is equal to end date", () => {
          it("calls addModifierToRange with `before-hovered-end`", () => {
            const addModifierFromRangeSpy = sinon.spy(
              DayPickerRangeController.prototype,
              "addModifierToRange"
            );
            const endDate = today;
            ...
            wrapper.instance().onDayMouseEnter(today);
            const addModifierFromRangeCalls = getCallsByModifier(
              addModifierFromRangeSpy,
              "before-hovered-end"
            );
            expect(addModifierFromRangeCalls.length).to.equal(1);
            expect(
              toISODateString(addModifierFromRangeCalls[0].args[1])
            ).to.equal(toISODateString(minimumNightStartSpan));
    ...
    describe("modifiers", () => {
      ...
      describe("startDate and !endDate and hoverDate is after startDate", () => {
        it("calls deleteModifierFromRange with startDate, hoverDate and `hovered-span`", () => {
          ...
          deleteModifierFromRangeSpy.resetHistory();
          wrapper.instance().onDayMouseLeave(today);
          const hoveredSpanCalls = getCallsByModifier(
            deleteModifierFromRangeSpy,
            "hovered-span"
          );
          ...
          expect(
            isSameDay(hoveredSpanCalls[0].args[2], dayAfterHoverDate)
          ).to.equal(true);
        });
      });
  ...
  describe("#onPrevMonthClick", () => {
    it("updates state.currentMonth to subtract 1 month", () => {
      ...
      expect(wrapper.state().currentMonth.month()).to.equal(
        today.clone().subtract(1, "month").month()
      );
    });
    ...
    it("new visibleDays does not have current last month", () => {
      ...
      wrapper.instance().onPrevMonthClick();
      const visibleDays = Object.keys(wrapper.state().visibleDays);
      expect(visibleDays).to.not.include(
        toISOMonthString(moment().add(numberOfMonths, "months"))
      );
    });

    it("calls this.getModifiers", () => {
      const getModifiersSpy = sinon.spy(
        DayPickerRangeController.prototype,
        "getModifiers"
      );
      const wrapper = shallow(
    ...
    it("calls props.onPrevMonthClick with new month", () => {
      ...
      expect(onPrevMonthClickStub.firstCall.args[0].year()).to.equal(
        newMonth.year()
      );
      expect(onPrevMonthClickStub.firstCall.args[0].month()).to.equal(
        newMonth.month()
      );
    });

    it("calls this.shouldDisableMonthNavigation twice", () => {
      const shouldDisableMonthNavigationSpy = sinon.spy(
        DayPickerRangeController.prototype,
        "shouldDisableMonthNavigation"
      );
      const wrapper = shallow(
        <DayPickerRangeController
          onDatesChange={sinon.stub()}
  ...
  describe("#onNextMonthClick", () => {
    ...
    it("new visibleDays does not have current month - 1", () => {
      ...
      const visibleDays = Object.keys(wrapper.state().visibleDays);
      expect(visibleDays).to.not.include(
        toISOMonthString(today.clone().subtract(1, "month"))
      );
    });
    ...
    it("calls props.onNextMonthClick with new month", () => {
      ...
      expect(onNextMonthClickStub.firstCall.args[0].year()).to.equal(
        newMonth.year()
      );
      expect(onNextMonthClickStub.firstCall.args[0].month()).to.equal(
        newMonth.month()
      );
    });
  });
  ...
  describe("#getFirstFocusableDay", () => {
    describe("focusedInput === START_DATE", () => {
      it("returns startDate if exists and is not blocked", () => {
        sinon
          .stub(DayPickerRangeController.prototype, "isBlocked")
          .returns(false);
        const wrapper = shallow(
          <DayPickerRangeController
            focusedInput={START_DATE}
        ...
        const firstFocusableDay = wrapper
          .instance()
          .getFirstFocusableDay(moment().subtract(10, "days"));
    ...
    describe("focusedInput === END_DATE", () => {
      ...
      it("returns first day of arg month if startDate and endDate are falsy", () => {
        ...
        const firstFocusableDay = wrapper
          ...
          .getFirstFocusableDay(today);
        expect(
          firstFocusableDay.isSame(today.clone().startOf("month"), "day")
        ).to.equal(true);
      });
    });
    ...
    describe("desired day is blocked", () => {
      it("returns next unblocked visible day after desired day if exists", () => {
        const isBlockedStub = sinon.stub(
          DayPickerRangeController.prototype,
          "isBlocked"
        );
        ...
        const firstFocusableDay = wrapper
          ...
          .getFirstFocusableDay(today);
        expect(
          firstFocusableDay.isSame(startDate.clone().add(9, "days"), "day")
        ).to.equal(true);
      });
    });
  });
  ...
  describe("#getModifiers", () => {
    it("return object has the same number of days as input", () => {
      ...
      expect(Object.keys(modifiers[monthISO]).length).to.equal(
        visibleDays[monthISO].length
      );
    });

    it("calls this.getModifiersForDay for each day in input", () => {
      const getModifiersForDaySpy = sinon.spy(
        DayPickerRangeController.prototype,
        "getModifiersForDay"
      );
      const monthISO = toISOMonthString(today);
      ...
      expect(getModifiersForDaySpy.callCount).to.equal(
        visibleDays[monthISO].length
      );
    });
  });
  ...
  describe("#getModifiersForDay", () => {
    it("only contains `valid` if all modifier methods return false", () => {
      ...
      const isDayHighlightedStub = sinon.stub().returns(false);
      sinon
        .stub(DayPickerRangeController.prototype, "isStartDate")
        .returns(false);
      sinon
        .stub(DayPickerRangeController.prototype, "isEndDate")
        .returns(false);
      sinon
        .stub(DayPickerRangeController.prototype, "doesNotMeetMinimumNights")
        .returns(false);
      sinon
        .stub(DayPickerRangeController.prototype, "isInSelectedSpan")
        .returns(false);
      sinon
        .stub(DayPickerRangeController.prototype, "isLastInRange")
        .returns(false);
      sinon
        .stub(DayPickerRangeController.prototype, "isHovered")
        .returns(false);
      sinon
        .stub(DayPickerRangeController.prototype, "isInHoveredSpan")
        .returns(false);
      sinon
        .stub(DayPickerRangeController.prototype, "isDayAfterHoveredStartDate")
        .returns(false);
      sinon
        .stub(DayPickerRangeController.prototype, "isFirstDayOfWeek")
        .returns(false);
      sinon
        .stub(DayPickerRangeController.prototype, "isLastDayOfWeek")
        .returns(false);
      const wrapper = shallow(
        <DayPickerRangeController
          onDatesChange={sinon.stub()}
  ...
  describe("#addModifier", () => {
    ...
    it("is resilient when visibleDays is an empty object", () => {
      ...
      const modifiers = wrapper.instance().addModifier({}, today);
      expect(Object.keys(modifiers[toISOMonthString(today)])).to.contain(
    ...
    it("return value now has modifier arg for day if was in first arg", () => {
      ...
      const modifiers = wrapper
        .instance()
        .addModifier(updatedDays, today, modifierToAdd);
      expect(Array.from(modifiers[monthISO][todayISO])).to.contain(
        modifierToAdd
      );
    });
    ...
    it("return value now has modifier arg for day if was in state", () => {
      ...
      const modifiers = wrapper
        .instance()
        .addModifier({}, today, modifierToAdd);
      expect(Array.from(modifiers[monthISO][todayISO])).to.contain(
        modifierToAdd
      );
    });
    ...
    it("return new modifier if vertically scrollable load more months", () => {
      ...
      const modifiers = wrapper
        .instance()
        .addModifier(updatedDays, nextMonth, modifierToAdd);
      expect(Array.from(modifiers[nextMonthISO][nextMonthDayISO])).to.contain(
        modifierToAdd
      );
    });
  });
  ...
  it("return value now has modifier arg for day after getting next scrollable months", () => {
    ...
    let modifiers = wrapper.addModifier(
      updatedDays,
      futureDateAfterMultiply,
      modifierToAdd
    );
    expect(Array.from(modifiers[monthISO][todayISO])).to.not.contain(
      modifierToAdd
    );
    wrapper.onGetNextScrollableMonths();
    modifiers = wrapper.addModifier(
      updatedDays,
      futureDateAfterMultiply,
      modifierToAdd
    );
  ...
  it("return value now has modifier arg for day after getting previous scrollable months", () => {
    ...
    let modifiers = wrapper.addModifier(
      updatedDays,
      pastDateAfterMultiply,
      modifierToAdd
    );
    expect(Array.from(modifiers[monthISO][dayISO])).to.not.contain(
      modifierToAdd
    );
  ...
  describe("#deleteModifier", () => {
    ...
    it("returns first arg if day is not visible", () => {
      ...
      const modifiers = wrapper
        .instance()
        .deleteModifier(updatedDays, moment());
      expect(modifiers).to.equal(updatedDays);
    });
    ...
    it("return value no longer has modifier arg for day if was in first arg", () => {
      ...
      const modifiers = wrapper
        .instance()
        .deleteModifier(updatedDays, today, modifierToDelete);
      expect(Array.from(modifiers[monthISO][todayISO])).to.not.contain(
        modifierToDelete
      );
    });
    ...
    it("return value no longer has modifier arg for day if was in state", () => {
      ...
      const modifiers = wrapper
        .instance()
        .deleteModifier({}, today, modifierToDelete);
      expect(Array.from(modifiers[monthISO][todayISO])).to.not.contain(
        modifierToDelete
      );
    });
    ...
    it("return new modifier if vertically scrollable load more months", () => {
      ...
      const modifiers = wrapper
        .instance()
        .deleteModifier(updatedDays, nextMonth, modifierToDelete);
      expect(
        Array.from(modifiers[nextMonthISO][nextMonthDayISO])
      ).to.not.contain(modifierToDelete);
    });
  });
  ...
  describe("#deleteModifierFromRange", () => {
    ...
    it("calls deleteModifier with modifier arg as modifier", () => {
      ...
      wrapper
        .instance()
        .deleteModifierFromRange({}, spanStart, spanEnd, modifier);
  ...
  describe("day modifier methods", () => {
      ...
      describe("state.startDate !== null", () => {
        ...
        describe("props.focusedInput === END_DATE", () => {
          it("returns true if arg is < props.minimumNights after props.startDate", () => {
            ...
            expect(
              wrapper.instance().doesNotMeetMinimumNights(testDate)
            ).to.equal(true);
          });
    ...
    describe("#isDayAfterHoveredStartDate", () => {
      it("returns true if arg startDate is hovered and arg is the day after the startDate", () => {
        ...
        expect(
          wrapper.instance().isDayAfterHoveredStartDate(testDate)
        ).to.equal(true);
      });
      ...
      it("returns false if arg is day after state.hoverDate and props.minimumNights is 0", () => {
        const testDate = moment(today).add(1, "days");
        const wrapper = shallow(
          <DayPickerRangeController startDate={today} minimumNights={0} />
        );
        wrapper.setState({
          hoverDate: today,
        });
    ...
    describe("#isHovered", () => {
      it("returns false if focusedInput is falsy", () => {
        const wrapper = shallow(
          <DayPickerRangeController focusedInput={null} />
        );
        wrapper.setState({
          hoverDate: today,
        });
    ...
    describe("#isInHoveredSpan", () => {
      describe("props.endDate === null", () => {
        it("returns true if arg is in between props.startDate and state.hoverDate", () => {
          ...
          const wrapper = shallow(
            <DayPickerRangeController startDate={today} endDate={null} />
          );
          wrapper.setState({
      ...
      describe("props.startDate === null", () => {
        ...
        it("returns false if arg is > props.endDate", () => {
          const endDate = moment(today).add(5, "days");
          const wrapper = shallow(
            <DayPickerRangeController startDate={null} endDate={endDate} />
          );
          wrapper.setState({
            hoverDate: today,
          });
    ...
    describe("#isInSelectedSpan", () => {
      it("returns true if props.startDate < arg < props.endDate", () => {
        const endDate = moment(today).add(5, "days");
        const wrapper = shallow(
          <DayPickerRangeController startDate={today} endDate={endDate} />
        );
        const testDate = moment(endDate).subtract(1, "days");
    ...
    describe("#isLastInRange", () => {
      ...
      beforeEach(() => {
        isInSelectedSpanStub = sinon.stub(
          DayPickerRangeController.prototype,
          "isInSelectedSpan"
        );
      });
      ...
      it("returns false if arg is not the day before props.endDate", () => {
        isInSelectedSpanStub.returns(true);
        const wrapper = shallow(
          <DayPickerRangeController endDate={moment(today).add(2, "days")} />
        );
        expect(wrapper.instance().isLastInRange(today)).to.equal(false);
      });
    });
    ...
    describe("#isBlocked", () => {
      ...
      beforeEach(() => {
        ...
        doesNotMeetMinimumNightsStub = sinon.stub(
          DayPickerRangeController.prototype,
    ...
    describe("#isToday", () => {
      ...
      it("returns false if tomorrow", () => {
        const wrapper = shallow(<DayPickerRangeController />);
        expect(
          wrapper.instance().isToday(moment(today).add(1, "days"))
        ).to.equal(false);
      });
      ...
      it("returns false if last month", () => {
        const wrapper = shallow(<DayPickerRangeController />);
        expect(
          wrapper.instance().isToday(moment(today).subtract(1, "months"))
        ).to.equal(false);
      });
    });
    ...
    describe("#isFirstDayOfWeek", () => {
      it("returns true if first day of this week", () => {
        const wrapper = shallow(<DayPickerRangeController />);
        expect(
          wrapper.instance().isFirstDayOfWeek(moment().startOf("week"))
        ).to.equal(true);
      });
      ...
      it("returns true if same day as firstDayOfWeek prop", () => {
        ...
        const wrapper = shallow(
          <DayPickerRangeController firstDayOfWeek={firstDayOfWeek} />
        );
        expect(
          wrapper
            .instance()
            .isFirstDayOfWeek(moment().startOf("week").day(firstDayOfWeek))
        ).to.equal(true);
      });
      ...
      it("returns false if not the first day of the week", () => {
        const wrapper = shallow(<DayPickerRangeController />);
        expect(
          wrapper.instance().isFirstDayOfWeek(moment().endOf("week"))
        ).to.equal(false);
      });
    });
    ...
    describe("#isLastDayOfWeek", () => {
      it("returns true if last day of week", () => {
        const wrapper = shallow(<DayPickerRangeController />);
        expect(
          wrapper.instance().isLastDayOfWeek(moment().endOf("week"))
        ).to.equal(true);
      });
      ...
      it("returns true if 6 days after firstDayOfWeek prop", () => {
        ...
        expect(
          wrapper
            .instance()
            .isLastDayOfWeek(moment().day(firstDayOfWeek).add(6, "days"))
        ).to.equal(true);
      });
      ...
      it("returns false if not last of week", () => {
        ...
        expect(
          wrapper
            .instance()
            .isLastDayOfWeek(moment().startOf("week").add(1, "day"))
        ).to.equal(false);
      });
    });
    ...
    describe("#beforeSelectedEnd", () => {
      it("returns true if day is before end date", () => {
        ...
        const wrapper = shallow(<DayPickerRangeController endDate={endDate} />);
        expect(wrapper.instance().beforeSelectedEnd(dayBeforeEndDate)).to.equal(
          true
        );
      });
      ...
      it("returns false if day is after or equal to end date", () => {
        ...
        const wrapper = shallow(<DayPickerRangeController endDate={endDate} />);
        expect(wrapper.instance().beforeSelectedEnd(dayAfterEndDate)).to.equal(
          false
        );
      });
    });
    ...
    describe("#isDayBeforeHoveredEndDate", () => {
      it("returns false if day is after hovered end date", () => {
        ...
        expect(
          wrapper.instance().isDayBeforeHoveredEndDate(dayAfterEndDate)
        ).to.equal(false);
      });
      ...
      it("returns true if day is before hovered end date", () => {
        ...
        wrapper.setState({ hoverDate: endDate });
        expect(
          wrapper.instance().isDayBeforeHoveredEndDate(dayBeforeEndDate)
        ).to.equal(true);
      });
    });
    ...
    describe("renderKeyboardShortcutsButton prop", () => {
      it("pass down custom button render function", () => {
        ...
        expect(dayPicker.prop("renderKeyboardShortcutsButton")).to.eql(
    ...
    describe("renderKeyboardShortcutsPanel prop", () => {
      it("passes down custom panel render function", () => {
        ...
        const dayPicker = wrapper.find(DayPicker);
        expect(dayPicker).to.have.lengthOf(1);
        expect(dayPicker.prop("renderKeyboardShortcutsPanel")).to.eql(
          testRenderKeyboardShortcutsPanel
        );
      });
    });
  });
