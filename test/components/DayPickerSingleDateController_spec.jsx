...
describe("DayPickerSingleDateController", () => {
      ...
      describe("selected modifier", () => {
        describe("props.date did not change", () => {
          it("does not call this.addModifier with `selected", () => {
            ...
            expect(
              getCallsByModifier(addModifierSpy, "selected").length
            ).to.equal(0);
          });
          ...
          it("does not call this.deleteModifier with `selected", () => {
            ...
            wrapper
              ...
              .UNSAFE_componentWillReceiveProps({ ...props, date });
            expect(
              getCallsByModifier(deleteModifierSpy, "selected").length
            ).to.equal(0);
          });
        });
        ...
        describe("props.date changed", () => {
          it("deleteModifier gets called with old date and `selected`", () => {
            ...
            wrapper
              .instance()
              .UNSAFE_componentWillReceiveProps({ ...props, date: newDate });
            const selectedCalls = getCallsByModifier(
              deleteModifierSpy,
              "selected"
            );
      ...
      describe("blocked", () => {
        ...
        describe("props.focused changed", () => {
          ...
          it("if isBlocked(day) is true calls addModifier with `blocked` for each day", () => {
            ...
            sinon
              .stub(DayPickerSingleDateController.prototype, "isBlocked")
              .returns(true);
            const wrapper = shallow(
              <DayPickerSingleDateController {...props} />
            );
  ...
  describe("#getFirstFocusableDay", () => {
    ...
    describe("desired date is blocked", () => {
      it("returns first unblocked visible day if exists", () => {
        ...
        expect(
          firstFocusableDay.isSame(date.clone().add(8, "days"), "day")
        ).to.equal(true);
      });
    });
  });
  ...
  describe("#getModifiersForDay", () => {
    it("only contains `valid` if all modifier methods return false", () => {
      sinon
        .stub(DayPickerSingleDateController.prototype, "isToday")
        .returns(false);
      sinon
        .stub(DayPickerSingleDateController.prototype, "isBlocked")
        .returns(false);
      ...
      const isDayHighlightedStub = sinon.stub().returns(false);
      sinon
        .stub(DayPickerSingleDateController.prototype, "isSelected")
        .returns(false);
      sinon
        .stub(DayPickerSingleDateController.prototype, "isHovered")
        .returns(false);
      sinon
        .stub(DayPickerSingleDateController.prototype, "isFirstDayOfWeek")
        .returns(false);
      sinon
        .stub(DayPickerSingleDateController.prototype, "isLastDayOfWeek")
        .returns(false);
      const wrapper = shallow(
        <DayPickerSingleDateController
          onDateChange={sinon.stub()}
  ...
  describe("#addModifier", () => {
    ...
    it("return value now has modifier arg for day after getting previous scrollable months", () => {
      const modifierToAdd = "foo";
      const numberOfMonths = 2;
      const pastDateAfterMultiply = today
        .clone()
  ...
  describe("modifiers", () => {
    ...
    describe("#isFirstDayOfWeek", () => {
      ...
      it("returns true if same day as firstDayOfWeek prop", () => {
        ...
        const wrapper = shallow(
          <DayPickerSingleDateController firstDayOfWeek={firstDayOfWeek} />
        );
        expect(
          wrapper
            .instance()
  ...
  describe("initialVisibleMonth", () => {
    ...
    describe("initialVisibleMonth is not passed in", () => {
      it("DayPickerSingleDateController.props.initialVisibleMonth evaluates to date", () => {
        ...
        const dayPicker = wrapper.find(DayPicker);
        expect(dayPicker.props().initialVisibleMonth().month()).to.equal(
          date.month()
        );
      });
      ...
      it("DayPickerSingleDateController.props.initialVisibleMonth evaluates to today if !date", () => {
        ...
        const dayPicker = wrapper.find(DayPicker);
        expect(
          dayPicker.props().initialVisibleMonth().isSame(today, "day")
        ).to.equal(true);
      });
    });
    ...
    describe("noNavButtons prop", () => {
      ...
      it("does not render navigation button when noNavButtons prop applied", () => {
        const wrapper = shallow(<DayPickerSingleDateController noNavButtons />)
          .dive()
          .dive();
