...
import DayPicker, {
  PureDayPicker,
  defaultProps as DayPickerDefaultProps,
} from "../../src/components/DayPicker";
...
describe("DayPicker", () => {
  ...
  describe("#render", () => {
    describe("renderWeekHeader", () => {
      ...
      describe("props.renderWeekHeaderElement", () => {
        it("there are 7 custom elements on each .DayPicker__week-header class", () => {
          ...
          const wrapper = shallow(
            <DayPicker
              renderWeekHeaderElement={(day) => (
                <strong className={testWeekHeaderClassName}>{day}</strong>
              )}
            />
          ).dive();
          const weekHeaders = wrapper.find(".DayPicker__week-header");
          weekHeaders.forEach((weekHeader) => {
            expect(
              weekHeader.find(`.${testWeekHeaderClassName}`)
            ).to.have.lengthOf(7);
          });
        });
      });
    ...
    describe("DayPickerNavigation", () => {
      it("is rendered before CalendarMonthGrid in DayPicker_focusRegion", () => {
        ...
        expect(
          wrapper.find('[className^="DayPicker_focusRegion"]').childAt(0).type()
      ...
      describe("navPosition === NAV_POSITION_BOTTOM", () => {
        it("is rendered after CalendarMonthGrid in DayPicker_focusRegion", () => {
          const wrapper = shallow(
            <DayPicker navPosition={NAV_POSITION_BOTTOM} />
          ).dive();
          expect(wrapper.find(DayPickerNavigation)).to.have.lengthOf(1);
          expect(
            wrapper
    ...
    describe("DayPickerKeyboardShortcuts", () => {
      ...
      it("component exists with custom button render function if renderKeyboardShortcutsButton is passed down", () => {
        ...
        const dayPickerKeyboardShortcuts = wrapper.find(
          DayPickerKeyboardShortcuts
        );
        expect(dayPickerKeyboardShortcuts).to.have.lengthOf(1);
        expect(
          dayPickerKeyboardShortcuts.prop("renderKeyboardShortcutsButton")
        ).to.eql(testRenderKeyboardShortcutsButton);
      });
      ...
      it("component exists with custom panel render function if renderKeyboardShortcutsPanel is passed down", () => {
        ...
        expect(dayPickerKeyboardShortcuts).to.have.lengthOf(1);
        expect(
          dayPickerKeyboardShortcuts.prop("renderKeyboardShortcutsPanel")
        ).to.eql(testRenderKeyboardShortcutsPanel);
      });
    });
  });
  ...
  describe("props.orientation === VERTICAL_SCROLLABLE", () => {
    ...
    it("uses getNextScrollableMonths instead of onNextMonthClick", () => {
      ...
      expect(nav.props.onNextMonthClick).to.equal(
    ...
    it("uses getPrevScrollableMonths instead of onNextMonthClick", () => {
      ...
      expect(nav.props.onPrevMonthClick).to.equal(
    ...
    describe("focusedDate is truthy", () => {
      ...
      describe("Tab", () => {
        it("triggers onShiftTab when shift tab is pressed", () => {
          ...
          wrapper
            .instance()
            .onKeyDown({ ...event, key: "Tab", shiftKey: true });
  ...
  describe("#onPrevMonthClick", () => {
    it("calls onPrevMonthTransition", () => {
      const onPrevMonthTransitionSpy = sinon.spy(
        PureDayPicker.prototype,
  ...
  describe("#onNextMonthClick", () => {
    it("calls onNextMonthTransition", () => {
      const onNextMonthTransitionSpy = sinon.spy(
        PureDayPicker.prototype,
  ...
  describe("#getFocusedDay", () => {
    describe("props.getFirstFocusableDay is truthy", () => {
      ...
      it("returns getFirstFocusableDay() value", () => {
        ...
        expect(
          wrapper.instance().getFocusedDay().isSame(today, "day")
        ).to.equal(true);
      });
  ...
  describe("#getPrevScrollableMonths", () => {
    it("increments scrollableMonthMultiple and updates currentMonth", () => {
      ...
      expect(wrapper.state().scrollableMonthMultiple).to.equal(2);
      expect(
        isSameMonth(wrapper.state().currentMonth, moment().subtract(2, "month"))
      ).to.equal(true);
    });
  });
  ...
  describe("#weekHeaderNames", () => {
    it("returns weekheaders in fr", () => {
      ...
      const wrapper = shallow(
        <DayPicker initialVisibleMonth={() => INITIAL_MONTH} />
      ).dive();
      const instance = wrapper.instance();
      expect(instance.getWeekHeaders()).to.be.eql(
        INITIAL_MONTH.localeData().weekdaysMin()
      );
    });
  });
  ...
  describe("#getWeekHeaders", () => {
    it("returns unmutated weekday headers for currentMonth in a future", () => {
      ...
      const getWeekHeadersSpy = sinon.spy(
        PureDayPicker.prototype,
        "getWeekHeaders"
      );
    ...
    describe("#UNSAFE_componentWillReceiveProps", () => {
      describe.skip("props.orientation === VERTICAL_SCROLLABLE", () => {
        it("updates state.currentMonthScrollTop", () => {
          ...
          const prevCurrentMonthScrollTop =
            wrapper.state().currentMonthScrollTop;
          wrapper.setState({
          ...
          expect(wrapper.state().currentMonthScrollTop).to.not.equal(
            prevCurrentMonthScrollTop
          );
        });
      });
    ...
    describe("#componentDidUpdate", () => {
      ...
      describe("props.orientation === HORIZONTAL_ORIENTATION", () => {
        ...
        it.skip("calls adjustDayPickerHeight if daySize has changed", () => {
          const wrapper = mount(
            <DayPicker daySize={39} orientation={HORIZONTAL_ORIENTATION} />
          );
          wrapper.setState({
            daySize: 40,
            orientation: HORIZONTAL_ORIENTATION,
      ...
      describe.skip("props.orientation === VERTICAL_ORIENTATION", () => {
        ...
        it("calls adjustDayPickerHeight if daySize has changed", () => {
          const wrapper = mount(
            <DayPicker daySize={39} orientation={VERTICAL_ORIENTATION} />
          );
          wrapper.setState({
            daySize: 40,
            orientation: VERTICAL_ORIENTATION,
      ...
      describe.skip("props.orientation === VERTICAL_SCROLLABLE", () => {
        it("does not update transitionContainer ref`s scrollTop currentMonth stays the same", () => {
          ...
          expect(wrapper.transitionContainer).to.have.property(
            "scrollTop",
            prevScrollTop
          );
        });
        ...
        it("updates transitionContainer ref`s scrollTop currentMonth changes", () => {
          ...
          expect(wrapper.transitionContainer).to.not.have.property(
            "scrollTop",
            prevScrollTop
          );
        });
